...
function overlapsBrick(bricks, fx, fy) {
...
function isInsideMap(x, y, w, h) {
    return x >= 0 && y >= 0 && x + FOREST_SECTION_SIZE <= w && y + FOREST_SECTION_SIZE <= h;
}
function forestKey(x, y) {
    return `${x}:${y}`;
}
function tryGenerateForestGroups(bricks, mapW, mapH, forestGroupsMax) {
    const forests = [];
    const used = new Set();
    const groupsCount = Math.floor(Math.random() * (forestGroupsMax - 1 + 1)) + 1;
    for (let groupIndex = 0; groupIndex < groupsCount; groupIndex++) {
        const groupSize = Math.floor(Math.random() * (FOREST_GROUP_SIZE_MAX - FOREST_GROUP_SIZE_MIN + 1)) + FOREST_GROUP_SIZE_MIN;
        let placed = false;
        for (let attempts = 0; attempts < 120 && !placed; attempts++) {
            ...
            if (!isInsideMap(sx, sy, mapW, mapH) || overlapsBrick(bricks, sx, sy))
                continue;
            if (used.has(forestKey(sx, sy)))
                continue;
            const group = [{ x: sx, y: sy }];
            const local = new Set([forestKey(sx, sy)]);
            ...
            while (group.length < groupSize && frontier.length > 0) {
                ...
                if (!isInsideMap(nx, ny, mapW, mapH))
                    continue;
                if (local.has(key) || used.has(key))
                    continue;
                if (overlapsBrick(bricks, nx, ny))
                    continue;
...
export function generateMapData(mapSize) {
    let mapW = MAP_WIDTH;
    let mapH = MAP_HEIGHT;
    if (mapSize === 'medium') {
        mapW = Math.round(MAP_WIDTH * 1.3);
        mapH = Math.round(MAP_HEIGHT * 1.3);
    }
    else if (mapSize === 'large') {
        mapW = Math.round(MAP_WIDTH * 1.6);
        mapH = Math.round(MAP_HEIGHT * 1.6);
    }
    const bricks = [];
    const biome = 0;
    const buildings = [];
    ...
    for (let i = 0; i < buildingCount; i++) {
        ...
        while (!valid && attempts < 50) {
            ...
            for (const other of buildings) {
                if (b.x < other.x + other.w + BRICK_SIZE &&
