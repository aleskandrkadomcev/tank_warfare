...
function parseScoreLimit(val) {
    const n = typeof val === 'number' ? val : typeof val === 'string' ? parseInt(val, 10) : NaN;
    if (!Number.isFinite(n))
        return MAX_SCORE;
    // берём ближайшее допустимое значение
    return SCORE_LIMITS.includes(n) ? n : MAX_SCORE;
}
export function handleCreateLobby(wss, ws, data) {
...
export function handleJoinLobby(wss, ws, data) {
...
export function handleUpdatePlayer(_wss, ws, data) {
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
    if (lobby && !lobby.gameStarted) {
        if (typeof data.nickname === 'string')
            ws.nickname = sanitizeNick(data.nickname);
        if (typeof data.color === 'string' && isValidColor(data.color))
            ws.color = data.color;
        // Хост может менять настройки лобби
        if (ws.id === lobby.hostId) {
            if (data.scoreLimit !== undefined)
                lobby.scoreLimit = parseScoreLimit(data.scoreLimit);
        }
        broadcastLobbyState(lobby);
    }
}
export function handleChangeTeam(_wss, ws, data) {
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
...
export function handleToggleReady(_wss, ws, _data) {
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
    if (lobby && !lobby.gameStarted) {
        ws.ready = !ws.ready;
        broadcastLobbyState(lobby);
...
export function handleStartGame(_wss, ws, data) {
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
...
export function handleAddBot(wss, ws, data) {
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
    if (!lobby)
        return;
    if (ws.id !== lobby.hostId) {
...
export function handleRemoveBot(wss, ws, data) {
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
    if (!lobby)
        return;
    if (ws.id !== lobby.hostId) {
