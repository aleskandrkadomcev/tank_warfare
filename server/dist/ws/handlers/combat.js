...
export function handleBullet(_wss, ws, data) {
...
export function handleBulletRemove(_wss, ws, data) {
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
    if (lobby?.gameStarted) {
        ...
        if (idx !== -1)
            lobby.aiBullets.splice(idx, 1);
        broadcastGame(lobby, { type: ServerMsg.BULLET_REMOVE, bulletId });
    }
}
export function handleDealDamage(_wss, ws, data) {
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
    if (lobby?.gameStarted) {
...
export function handleCollisionDamage(_wss, ws, data) {
    if (!data.otherId)
        return;
    const lobby = ws.lobbyId ? lobbies[ws.lobbyId] : undefined;
    if (lobby?.gameStarted) {
